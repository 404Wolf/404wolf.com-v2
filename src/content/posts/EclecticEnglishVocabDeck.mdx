...
date: '2023-01-01'
covers:
  - 'https://static.404wolf.com/mobile_flashcards.webp'
  - 'https://static.404wolf.com/desktop_flashcard_front.webp'
  - 'https://static.404wolf.com/word_cubes.webp'
description: |
  Conglomeration, obstreperous, mercurial, preantepenultimate, rapacious,
...
links: [["Github", "https://github.com/404wolf/logophilio"]]
---

# What it Is

This project discusses my eclectic, carefully curated deck of English vocab
...
Webster dictionary, and have also put together images and audio for each card.
...
As I encounter new, unknown words, I've stored them away in a
[Google Keep](https://keep.google.com/) backlog. I've gotten into the habit of
...
[spaced-repetition](https://en.wikipedia.org/wiki/Spaced_repetition) flashcard
deck.
...
Spaced repetition is a special evidence-based flashcard studying method wherein
...
![Leitner box system](https://static.404wolf.com/Leitner_system_animation_0002.gif)

Generally these days spaced repetition is implemented with software, but that
does not necessarily need to be the case.
['Leitner boxes'](https://en.wikipedia.org/wiki/Leitner_system) are an
...
Although approaches like this do work, I highly recommend using software to
...
[Anki Flashcards](https://apps.ankiweb.net/). It's completely open source, easy
...
android app, but their iPhone app is $25 (and worth every penny).

A copy of the deck can be downloaded
[here](https://drive.google.com/file/d/1wysbw8pV2i1xhy1Xwt3zRoyVMkC2zH5s/view?usp=share_link).
...
With my deck building up in size, and my appetite for words evergrowing, I knew
...
help create the cards.

The first step of figuring out how to automate would be to determine where and
how to fetch card information. While indeed many dictionary APIs, including free
options, already exist, such as [webster's](https://dictionaryapi.com/), I
...
Over the past month, I've come to find that OpenAI's ChatGTP has been
...
Google text to speech, given it's robustness and popularity.

With ChatGTP's API, you have the option of providing a "system message" to the
...
Images also require a prompt, but for the image I decided that the best person
to write a prompt would AI itself. So, for the images, I prompted GPT to write a
prompt for DALLE using the following template.
...
After using Anki for a while for this project though, I decided that it'd be fun
...
which I write more about [here](https://404wolf.com/posts/blog/svgShenanigans).
...
for more accurate to the word images. Here's some examples...
...
Although the software tool I initially was working on was a basic script to help
...
be found [here](https://github.com/404wolf/logophilio), but I'm in the process
...
While my API implementation enables concurrent flashcard generation server-side,
...
could then use to get the generated flashcards later.

For this, I've been looking into using some sort of task queue system with
...
to a dedicated server always having a chromedriver running.
...
The next step for this chapter of the project, after further refining the API,
