...
type: 'project'
date: '2023-01-01'
covers: ['https://static.404wolf.com/remarkableExample_0001.JPG']
description: |
  A remarkable tablet is a digital paper replacement: an e-ink screen tablet
...
![My remarkable](https://static.404wolf.com/remarkableExample_0001.JPG)
...
Recently I got very into using a [remarkable](https://remarkable.com) tablet,
...
semester at Case as an alternative for paper, and it's been great.

One problem I have with the tablet, however, is how proprietary their ecosystem
...
This seems a bit hypocritical, but for the convenience their cloud offers I
...
So, since the cloud is not a good backup instrument, the issue is that they do
...
Before I begin to discuss my tool, it's important to understand how notebooks
...
metadata files like the last-page or notebook name and orientation.

Preserving the actual notebook files as .rmn files is important because if the
...
notebook (for example, you can't move around marker marks, erase regions, etc).

But of course, exporting PDFs is important to me too, since one of the goals of
...
create a simple recovery backup, and then also a second backup with PDFs.
...
![Mobile App](https://static.404wolf.com/mobileApp_0001.PNG)

So, let's talk about how exporting works with the tablet natively. Exporting
...
Though the desktop software provides a great user interface and the ability to
...
contents and run rmn to PDF conversions.

It's unideal to have to render the notebooks on the tablet itself, since it
...
time. My plan was to just run the backup overnight.

### Access the Files

![SSH Access](https://static.404wolf.com/sshAccess_0001.png)

It's a bit strange, but Remarkable (the company) does not want you to know that
...
download button just exports the notebook as a PDF. The rmn files are invisible.

So, then, how do we even know that rmn files exist? Remarkable uses some open
...
directly off the tablet, with access to root ssh.
...
[github](https://github.com/404Wolf/RemarkableBackup), and is functional.

#### Future Plans

Now that I have a working backup script, my future plans for this project are to
...
copies of the rmns in case anything happens to their cloud or the tablet.

# E-Ink

![How e-ink works](https://static.404wolf.com/einkDemo_0001.gif)

Something that I decided to research as I was tackling this project was how the
...
crisp black or white spot.

What's unique about e-ink is that it doesn't require a backlight like RGB LCD or
...
screen and specially designed stylus with particular pen nibs.
