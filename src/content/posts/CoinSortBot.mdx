...
covers:
  [
    'https://static.404wolf.com/sideViewWithRamp_0001.png',
    'https://static.404wolf.com/frontView_0001.png',
    'https://static.404wolf.com/wiresOnTable_0001.png',
  ]
description: |
  Coin-sort-bot is a physical robot that I co-designed and wrote software for to
...
tags: ['academic']
links: [["Github", "https://github.com/Coin-Sort-Bot"]]
---
...
A while back I was sifting through some older coins that my grandma had saved
...
then were made from zink.

Looking at "coin identifying" apps on the app store, I found that there were
...
couldn't find a good API for the process.

Additionally, though these apps presented a software solution to make manually
sifting through the coins more streamlined, I still found it tedious to have to
lay down each coin on the table one by one, and snap photos of each side.

I decided then, that, in addition to making a tool like that of the apps to
...
rolls based on their year and/or value.

## Think\[box] Grant

At my university (Case Western) there is a campus makerspace called "Thinkbox."
Each year they award various project ideas up to $2.5k to bring the ideas to
fruition, through the
[Thinkbox project fund](https://case.edu/thinkbox/funding/student-project-fund).
Their grant page lists projects eligible as ones that are:
...
Which my automatic coin sorter was a perfect match for. I applied for the grant,
...
Towards the end of my first semester, I attended our senior project fair, and,
...
Early on we decided to make the project mostly 3D printed, to make fabrication
...
unique facet of our design, where we decided we'd let hang off the table).

A lot of the initial steps for the process were drafting ideas on paper, trying
...
sift through very large amounts of coins.

Most of the components 3D printed were designed in Fusion360, because of how
...
We got Servos and most of our other parts through Adafruit. Adafruit makes
coding for their controller boards super easy by providing libraries that 'just
work.' Check out their motor controller board example code, for example...
...
What _was_ annoying, however, was getting RPIO (the interface that lets you use
...
To network to the Pi, originally I set up a simple cron job to automatically
...
[tailscale](https://tailscale.com/), which has been absolutely amazing and
...
simple as a single `curl` command.

![Digital microscope coin image](https://static.404wolf.com/microscopeImage_0001.jpg)

For the main board, we decided to go with a $50 Raspberry Pi 4 (which we were
...
The controllers were annoying to set up since they are designed to be "HATs" for
...
the bottom servo hat to allow the wires to stick out for the servos.

## Initial design

![Coin sorter inspiration](https://static.404wolf.com/ElectricSorterLowAngleCoins061617_0001.jpg)

For the during school year portion of the project, I decided to work
...
the coins to proper output coin rolls.
...
We took inspiration from the a classic metal coin sorter with a rotated wheel,
...
The next step for the spinning drum design was to "slice" the coins off the
...
them into view of a camera, and then, afterwards, separate them.

![Beam break sensor](https://static.404wolf.com/2168-04_0001.jpg)

We quickly realized, though, that it was entirely possible that multiple coins
...
sensor, and so we ordered [this one](https://www.adafruit.com/product/2168) from
Adafruit. But before we even tested it, we realized that adding a new chamber to
limit the flow of coins would be a whole new challenge in it of itself.

![The chute](https://static.404wolf.com/Screenshot_from_2024-04-25_23-55-17_0001.png)

We decided to take a break from the drum design, and move on to designing the
...
servo move into position so as to _redirect_ the current falling coin into a
...
This system worked great! The servo was able to redirect the coin into the
...
and move away from the spinning drum method.

## Redone design

![New design dispensing coins one at a time](https://static.404wolf.com/IMG_8408_0001_0001.webm)

Instead of having a spinning drum with room for multiple coins to be dispensed
...
The new design clamps on to the corner of a table, and the actual sorting chute
...
of the table), which is something we may consider later.

For our final project submission we provided the following rendering, since the
...
Although the actual processing of coin images is a later step that I haven't
...
[software that will live on the board itself](https://github.com/Coin-Sort-Bot/OnBoard/),
...
components, so that I can write code like this to control it.
...
What I've done is added another layer of abstraction, so that we can control
exactly what we need. For the cameras, I'm using `opencv`, which is a library
for processing images, that also has the ability to interface with the OS to
capture images. I experimented with `ffmeg` to capture the images by shelling
...
`opencv`. I may return to this eventually.

## Next steps

I'm currently brainstorming how to go about controlling the board remotely, and
am toying with the idea of using [MTQQ](https://mqtt.org/),
[RabbitMQ](https://rabbitmq-website.pages.dev/), or a simple websocket or
...
coins to fall down the proper chute.

In the future, the plan is to actually implement OCR to scan the dates off of
...
Ultimately, the prototyping and iterative design required for this project has
...
model, and licensing model).

# Additional Notes

This is still a work in progress! This post will continue to be updated.
