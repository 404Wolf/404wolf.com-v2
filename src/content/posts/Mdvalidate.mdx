...
and *extract* arbitrary pieces of information from them. Right now, it's all
packaged into a CLI, but evenutally I'd like to make it a Rust and Typescript
(wasm) library.
...
Let's look at a very basic example. To start, all "mdschema" files with vanilla
Markdown validate themselves. So
...
Validates, but produces no output. An input document will validate itself! We
can insert "matchers" into the Markdown, which are inline code spans with
special internal content, to "capture" text:
...
You use repeating matchers, specifying `{,}` to say "as many list items of this
form as you want."
...
It turns out streaming is really complicated to implement, and requires
validating trees that aren't totally well formed yet. So there are probably
some bugs lurking here.

# Making it!

`mdvalidate` is a rust project made with the help of
[treesitter](https://tree-sitter.github.io/tree-sitter/) for incremental
parsing and [this](https://github.com/ikatyang/tree-sitter-markdown) Markdown
...
quickly recompute trees without starting from scratch. I'm also using
[ariadne](https://docs.rs/ariadne/latest/ariadne/), a super cool pretty printer
Rust crate!
...
<SchemaAndInput
  schema={`document <-- matches ✔
└─ atx_heading <-- matches ✔
   ├─ atx_h1_marker <-- matches ✔
   └─ heading_content <-- matches ✔
      └─ text <-- matches ✔`}
  input={`document <-- matches ✔
└─ atx_heading <-- matches ✔
   ├─ atx_h1_marker <-- matches ✔
   └─ heading_content <-- matches ✔
      └─ text <-- matches ✔`}
