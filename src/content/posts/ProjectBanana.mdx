...
<div className="float-right w-[30%] ml-4">
<Mermaid
  code={`
  stateDiagram-v2
      [*] --> DecideToBustCache
      ...
      DecideToBustCache --> InvalidateCache : Local
      ReceiveCacheBust --> InvalidateCache : External

      InvalidateCache --> NotifyOthers : Broadcast
      NotifyOthers --> [*]

`} />

Either we get a request to edit a Val, which causes us to bust the cache and
then advertise to other servers, or we receive a message from another server to
bust the cache.
...
Let's say that if we are a worker, and we receive a request, and we look and see
that there's a unix socket file in a folder that has the literal name being the
hostname of the request we received, we are allowed to proxy. If we don't, then
we can delegate to a primary process that's allowed to talk to the database,
which can do the actual spawning, while we wait for it to finish and the needed
...
code = {`
  graph TD
      ...
      B2 -.->|Ask the primary to boot the worker, which will cause a unix socket file to exist| B1
