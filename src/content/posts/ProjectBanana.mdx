...
Project Banana was an exploratory project I worked on at Val Town to help allow
...
fastify/nodejs backend with minimal changes.
...
_everyone's_ Vals' executions, so it is possible that if one person's Val
...
---
 ...
 <div className="flex-1">
    <h4>Concurrent (Banana) 🍌 <span className='text-sm'>(round-robin)</span></h4>
    <Mermaid
      code={`
      graph TD
...
<Mermaid
  code={`
    graph TD
    %% Requests
    R1["Initial Cold Request --> a.com (arrives first)"]
    R2["Already Warming Request --> a.com (arrives during spawn)"]

    %% NodeJS processes
    N0["NodeJS Primary (boot info + cache)"]
    N1["NodeJS Child #1"]
    N2["NodeJS Child #2"]
    N3["NodeJS Child #3"]
    ...
    %% File system
    subgraph FS["File System (Unix Sockets)"]
        F1["a.com.sock"]
        F2["b.com.sock"]
        F3["c.com.sock"]
    end
    ...
    %% Workers
    subgraph Workers["Deno Worker Processes"]
        W1["Deno Worker for a.com"]
        W2["Deno Worker for b.com"]
        W3["Deno Worker for c.com"]
    end

    %% Cold start flow
    R1 --> N1
    N1 -->|"Ask boot info if not in /cache/a.com.json (for example)"| N0
    N0 -->|Query boot info| DB
    DB -->|"Return boot info and put in /cache/a.com.json (for example)"| N0
    N0 -->|Responds| N1
    N1 -->|Spawn worker & create socket| F1
    F1 --> W1

    %% Warm request arriving during spawn
    R2 --> N2
    N2 -.->|Wait for a.com.sock to appear| F1

    %% Warm request that's already running
    R3["Warm Request --> b.com (happy path)"] --> N3
    N3 -->|Proxy to b.com.sock| F2
    F2 --> W2

    %% Styling
    classDef primary fill:#e1f5fe,stroke:#0288d1
    classDef child fill:#f3e5f5,stroke:#7b1fa2
    classDef file fill:#e8f5e8,stroke:#388e3c
    classDef worker fill:#fff3e0,stroke:#f57c00
    classDef request fill:#e0f7fa,stroke:#006064
    classDef db fill:#ede7f6,stroke:#5e35b1

    class N0 primary
    class N1,N2,N3 child
    class F1,F2,F3 file
...
Like we said above, in the diagram there's three example requests, and they
arrive in this timeline:

1. An initial cold request for a.com, which requires spawning a new worker. In
  ...
  child process spawns the worker, creating the unix socket file.
1. An already warming request for a.com, which arrives while the first request
  is still spawning the worker. This process sees that there's no socket file
  ...
  spawning it).
1. A warm request for b.com, which is the happy path. The child process sees
  that there's already a socket file for b.com, and so it just proxies to it.

## Going in Parallel

Okay so we have a general idea of how the system will work, at a high level. But
how do we implement something like this -- a proxying setup -- in, err, server
side javascript?
...
Nodejs has the same thing! But it's built into the language, not a CLI feature.
...
<Collapsible title="And, for good measure, Bun has its own super cool, Linux way to do this!">
...
Here's the examples from [their docs](https://bun.com/docs/guides/http/cluster):
...
And Bun also supports `node:cluster`!
[(Deno does **not**)](https://docs.deno.com/api/node/cluster/).

</Collapsible>

And, since we have control of the code that the primary runs versus the workers,
...
And the actual worker server (the processes we're spawning, which happen to also
be javascript (Deno)) looks like this,
...
If a deno process dies, then all the nodejs processes will see the socket file
go away and clean up their own memory state.
...
export class DenoContainer extends EventEmitter<ContainerEvents> {
  ...
  /**
   * Readable stream of the container's running modules' stderr (from the supervisor process).
   */
  ...
  public async spawn() {
    const DENO_DIR = process.env.DENO_DIR;
  ...
  public async stop() {
    if (this.#proc) {
      this.#proc.kill("SIGUSR1"); // Kill the child, but keep the docker container running
    }
...
export class DenoContainerPool {
  ...
  async #preloadContainers(count: number): Promise<void> {
    const containers = await this.#getNContainers(count);
  ...
  async #getNContainers(n: number): Promise<DenoContainer[]> {
    return Promise.all(
      Array.from({ length: n }, async () => {
        const container = new DenoContainer({
          containerTag: this.#containerTag,
          socketsDir: this.#socketsDir,
        });
        await container.spawn();
