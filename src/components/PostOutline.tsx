...
export function PostOutline({ headings, expand = false, outlineItemClassName, containerClassName }: Props) {
...
function OutlineItem({
  ...
  const handleClick = (e: React.MouseEvent<HTMLAnchorElement>) => {
    ...
    if (element) {
      // 21% of viewport above the element, so it enters into the "ACTIVE REGION"
      const offset = window.innerHeight * 0.21;
      const elementPosition = element.getBoundingClientRect().top;
      const offsetPosition = elementPosition + window.pageYOffset - offset; // Element is at top of screen
  ...
  if (!children) {
    return (
      <a
        href={`#${slug}`}
        onClick={handleClick}
        className={styles.postOutline.leafItem({ isActive, className })}
      >
  ...
  return (
    <details open={expand} className={className}>
      <summary className={styles.postOutline.summary()}>
        <ChevronRight className={styles.postOutline.chevron()} />
        <a href={`#${slug}`} onClick={handleClick} className={styles.postOutline.link({ isActive })}>
          {text}
        </a>
      </summary>
